...
print(f'\n{"="*70}')
print(f'{"指标":<20} {"vc(×1.0)":>15} {"vc(×0.97)":>15} {"差值":>10}')
print('-'*60)
...
print(f'\n{"="*80}')
print(f'符号反转 (vc_full正→vc_adj负, 或反之): {len(reversals)} 个 ({len(reversals)/len(samples_1p0)*100:.2f}%)')
print(f'{"="*80}')
if reversals:
    print(f'{"代码":<14} {"名称":<8} {"日期":<10} {"vc×1.0":>9} {"vc×0.97":>9} {"D-2量":>10} {"D-3量":>10} {"方向"}')
    for code,name,d1,v1,v2,v2f,v3 in reversed(sorted(reversals, key=lambda x: abs(x[3]-x[4]))[:25]):
...
for lo,hi in zip(bins[:-1],bins[1:]):
    ...
    bar='#'*int(pct)
    print(f'  [{lo:>4.1f}~{hi:>4.1f}%): {cnt:>5} ({pct:>5.1f}%) {bar}')

# 每日排名影响
print(f'\n{"="*80}')
print('每日排名变化 (仅考虑vol_contract维度)')
print(f'{"="*80}')
...
for d1 in sorted(daily_meta.keys()):
    ...
    total_days+=1
    by_full=sorted(items,key=lambda x:-x[2])
    by_adj=sorted(items,key=lambda x:-x[3])
    if any(by_full[i][0]!=by_adj[i][0] for i in range(min(len(by_full),len(by_adj)))):
        rank_changes+=1
        if len(details)<5: details.append((d1,by_full,by_adj))

print(f'总天数(>=2候选): {total_days}')
print(f'vol_contract排位变化的天: {rank_changes} ({rank_changes/total_days*100:.1f}%)')
print()
for d1,bf,ba in details:
    print(f'  {d1}:')
    print(f'    ×1.0: ' + ' > '.join(f'{n}({vc:+.1f})' for c,n,vc,_,_,_ in bf))
    print(f'    ×0.97: ' + ' > '.join(f'{n}({vc:+.1f})' for _,n,_,vc,_,_ in ba))
    print()
...
print(f'标准化参数变化:')
print(f'  ×1.0:  MU={mu1:.2f}  SG={sg1:.2f}')
print(f'  ×0.97: MU={mu2:.2f}  SG={sg2:.2f}')
print(f'  MU偏移: {(mu2-mu1)/sg1*100:.1f}% SG')
print(f'  SG变化: {(sg2-sg1)/sg1*100:+.1f}%')
print(f'\n结论: 3%尾盘量对vol_contract的影响平均仅 {(mu2-mu1):.2f}个百分点, ')
print(f'      MU偏移{sg1:.1f}%标准差里占{(mu2-mu1)/sg1*100:.1f}% — {"几乎可以忽略" if abs(mu2-mu1)/sg1<0.02 else "需关注"}')
