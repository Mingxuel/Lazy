        ...
        private void UpdateStrategyDetailVWAP()
        {
            ...
            foreach (var i in Enumerable.Range(0, 1))
            {
                ...
                foreach (var date in TradingDates.Dates)
                {
                    ...
                    {
                        var values = ZZ5005M.Instance.GetM5Ratio(strategy_items[index].StockCode!, strategy_items[index].Date!);
                        ratio += (values[1] / values[0] - 1.0) * 100.0;

                        StrategyDetailVWAPTicketItem ticket_item = new StrategyDetailVWAPTicketItem();
                        ticket_item.Date = date;
                        ticket_item.StockCode = strategy_items[index].StockCode;
                        ticket_item.StockName = strategy_items[index].StockName;
                        ticket_item.CloseRatio = strategy_items[index].CloseRatio;
                        ticket_item.HighRatio = strategy_items[index].HighRatio;
                        ticket_item.LowRatio = strategy_items[index].LowRatio;
                        ticket_item.OpenRatio = strategy_items[index].OpenRatio;
                    ...
                    ratio = ratio / strategy_items.Count;
