...
功能：
    ...
    2. 显示 UI    —— 生成策略看板并在浏览器中打开
    3. QMT 自动交易 —— 买入 / 卖出监控 / 常驻 watch
    4. 上传 Gitee  —— 提交改动并推送到 Gitee 仓库（remote: gitee）
    5. 上传 GitHub —— 提交改动并推送到 GitHub 仓库（remote: github）
    0. 退出
执行完任一项后自动回到主菜单。
"""
...
def _menu_loop():
    while True:
        ...
        print(f"  {G}2{R}  {B}显示看板{R}    生成并打开策略 UI")
        print(f"  {G}3{R}  {B}QMT 自动交易{R} 买入 / 卖出监控 / 常驻 watch")
        print(f"  {G}4{R}  {B}上传 Gitee{R}    提交改动并推送到 Gitee 仓库")
        print(f"  {G}5{R}  {B}上传 GitHub{R}   提交改动并推送到 GitHub 仓库")
        ...
        if choice == "1":
            ...
            print(f"  {Y}无效选择，请输入 1 / 2 / 3 / 4 / 5 / 0。{R}")
