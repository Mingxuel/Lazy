...
consec=0;mm=defaultdict(list);yy=defaultdict(list)
...
for d1_date in ad:
    ...
    ret=fee(bp,sp,cap)
    mm[d1_date[:6]].append(ret)
    yy[d1_date[:4]].append(ret)
...
for y in sorted(yy.keys()):
    ...
    cv2*=yr
    wins=sum(1 for rv in yy[y] if rv>0)
    nall=len(yy[y])
    print(f'{y}: +{(yr-1)*100:.1f}%  胜率{wins}/{nall}({wins/nall*100:.1f}%)  笔数{nall}  净值{cv2:.2f}')

print()
print(f'{"月":<8} {"2025收益":>12} {"2026收益":>12}')
for m in ['01','02','03','04','05','06','07','08','09','10','11','12']:
    r25=mm.get('2025'+m,[]);r26=mm.get('2026'+m,[])
    v25=1.0;v26=1.0
    for rv in r25:v25*=(1+rv/100)
    for rv in r26:v26*=(1+rv/100)
    s25=f'{(v25-1)*100:+.1f}%({len(r25)})' if r25 else '—'
    s26=f'{(v26-1)*100:+.1f}%({len(r26)})' if r26 else '—'
    print(f'{m:<8} {s25:>12} {s26:>12}')
