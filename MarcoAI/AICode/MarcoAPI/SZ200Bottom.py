    ...
    for trading_date in trading_dates:
        ...
        with open(bottom_file, "w") as file:
            for stock_code in stock_codes:
                ...
                if _CALCULATE_BOTTOM(close, pre_close):
                    file.write(f"{stock_code}\n")

def _CALCULATE_BOTTOM(close: str, pre_close: str):
    _close: float = float(close)
    _pre_close = float(pre_close)
    decimal = Decimal(float(_pre_close) * 0.9)
    ...
    return abs(_close - _limit_price) < 0.001
